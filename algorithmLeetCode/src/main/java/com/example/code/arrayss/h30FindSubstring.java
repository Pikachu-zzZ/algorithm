...
public class h30FindSubstring {
    /**
     * 使用哈希表 map 记录 words 中每个单词的出现次数
     * 枚举 s 中的每个字符作为起点，往后取得长度为 length 的子串 sub
     * 使用哈希表 cur 统计 substring 每个单词的出现次数（每隔 len 长度作为一个单词）
     * 比较 cur 和 map 是否相同
     * @param s
     * @param words
     * @return
    ...
    public List<Integer> findSubstring(String s, String[] words) {
        ...
        for (int i = 0; i <= sLen - length; i++) {
            ...
            for (int j = 0; j < length; j+=len) {
                String item = substring.substring(j, j + len);
                if (!map.containsKey(item)) break;
